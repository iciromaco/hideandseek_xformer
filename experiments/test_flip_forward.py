...
with open(OUT, "w") as fo:
    ...
    while not done and step < 500:
        ...
        seq = env._get_policy_history_seq(
            ...
            env._inference_seq_lens.get(env.learnable_agent_key, seq_len),
            norm,
        )
        import torch
        ...
        f, t, lck, grb = (
            float(arr[0]),
            float(arr[1]) if arr.shape[0] > 1 else 0.0,
            float(arr[2]) if arr.shape[0] > 2 else 0.0,
            float(arr[3]) if arr.shape[0] > 3 else 0.0,
        )
        f = -f
