...
try:
    ...
    print('INITIAL agents:', [(k, env.data.xpos[env.body_ids[k]][:3].tolist()) for k in env.agent_keys])
...
print('agents:', [(k, env.data.xpos[env.body_ids[k]][:3].tolist()) for k in env.agent_keys])
print('qpos_indices keys:', list(getattr(env, 'qpos_indices', {}).keys()))
print('qpos_indices sample:', {k: getattr(env, 'qpos_indices', {}).get(k) for k in env.agent_keys})
