...
if state is not None:
    try:
        import re
        m = re.search(r'HNS_V27_seeker_s(\d+)_h(\d+)_b(\d+)_r(\d+)\.pt', os.path.basename(inferred_path))
        if m:
            inferred_cfg = {
    ...
    while not done and step < 500:
        ...
        if vis_flag or info.get('dbg_learnable_hider_seen', False):
            ...
            if env._inference_models.get(env.learnable_agent_key) is not None:
                ...
                seq = env._get_policy_history_seq(env.learnable_agent_key, env._inference_seq_lens.get(env.learnable_agent_key, 8), norm)
                import torch
