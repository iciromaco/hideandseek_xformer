import os
import json
import numpy as np
import matplotlib
matplotlib.use('Agg')
import matplotlib.pyplot as plt

hold_vals = ['-1.0','-0.5','-0.25','0.25','0.5','1.0']
jsons = [f'experiments/step_response_nowalls_h{v.replace("-","m").replace(".","p")}.json' for v in hold_vals]
labels = [f'h={v}' for v in hold_vals]

out_png = 'experiments/plots/vx_overlay_all_cases.png'
out_png_zoom = 'experiments/plots/vx_overlay_all_cases_zoom_250_320.png'
trim = 80
os.makedirs(os.path.dirname(out_png), exist_ok=True)

plt.figure(figsize=(10,5))
for j, lab in zip(jsons, labels):
    if not os.path.exists(j):
        print('missing', j)
        continue
    with open(j,'r') as f:
        data = json.load(f)
    recs = data.get('records', [])
    vx = [r.get('vx', 0.0) for r in recs]
    if len(vx) <= trim:
        continue
    vx_trim = vx[trim:]
    t = np.arange(len(vx_trim)) + trim
    plt.plot(t, vx_trim, label=lab, linewidth=1.5)

plt.xlabel('step')
plt.ylabel('vx (signed forward)')
plt.title('VX overlay: all hold cases (trimmed)')
plt.legend(ncol=2)
plt.grid(True)
plt.tight_layout()
plt.savefig(out_png)
print('Saved', out_png)

# zoom
lo, hi = 250, 320
plt.figure(figsize=(10,4))
for j, lab in zip(jsons, labels):
    if not os.path.exists(j):
        continue
    with open(j,'r') as f:
        data = json.load(f)
    recs = data.get('records', [])
    vx = [r.get('vx', 0.0) for r in recs]
    idx = [i for i in range(len(vx)) if lo <= i <= hi]
    if not idx: continue
    s_steps = [i for i in idx]
    s_vx = [vx[i] for i in idx]
    plt.plot(s_steps, s_vx, label=lab, linewidth=1.5)

plt.xlabel('step')
plt.ylabel('vx (signed forward)')
plt.title(f'VX overlay zoom {lo}-{hi}')
plt.legend(ncol=2)
plt.grid(True)
plt.tight_layout()
plt.savefig(out_png_zoom)
print('Saved', out_png_zoom)
