...
def pretty_print(obs, env):
    ...
    s = idx.SELF.SLICE
    print('\n-- SELF --')
    print(f'VEL_X={obs[s.start + idx.SELF.VEL_X]:.4f}', f'VEL_Y={obs[s.start + idx.SELF.VEL_Y]:.4f}', f'ROT={obs[s.start + idx.SELF.ROT]:.4f}', f'COS_ROT={obs[s.start + idx.SELF.COS_ROT]:.4f}', f'SIN_ROT={obs[s.start + idx.SELF.SIN_ROT]:.4f}')
...
if __name__=='__main__':
    ...
    print('sample info:', {k: info.get(k) for k in ['dbg_seek_gaze_cos_front_max','dbg_seek_gaze_cos_front_dist_max','dbg_learnable_hider_seen']})
