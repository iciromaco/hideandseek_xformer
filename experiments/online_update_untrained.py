...
def evaluate_policy(agent, env, seq_len=8, steps=80, render=False):
    ...
    try:
        vis = env.vis_engine.is_visible(
            np.asarray([s_pos[0], s_pos[1], 0.4]),
            np.asarray([hx, hy, 0.4]),
            mode=1,
            body_exclude=env.body_ids.get(s_key, -1),
            target_body_id=env.body_ids.get(h_key, -1),
        )
...
def main():
    ...
    try:
        vis2 = env.vis_engine.is_visible(
            np.asarray([s_pos[0], s_pos[1], 0.4]),
            np.asarray([hx, hy, 0.4]),
            mode=1,
            body_exclude=env.body_ids.get(s_key, -1),
    ...
    while steps < 1000 and len(collected) < rollout_len:
        ...
        try:
            ...
            try:
                vis_now = env.vis_engine.is_visible(
                    np.asarray([s_pos[0], s_pos[1], 0.4]),
                    np.asarray([h_pos[0], h_pos[1], 0.4]),
                    mode=1,
                    body_exclude=env.body_ids.get(s_key, -1),
                    target_body_id=env.body_ids.get(h_key, -1),
                )
    ...
    out = {
        "pre_mean_toward_dot": pre_score,
        "post_mean_toward_dot": post_score,
        "num_rollout": len(collected),
    }
