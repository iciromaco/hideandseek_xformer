...
def main(steps=400, show_progress=True):
        ...
        try:
                    ...
                    if inferred_hidden is None:
                        for k, v in model_state.items():
                            if k.startswith('obs_encoder') and k.endswith('.weight') and v.ndim == 2:
                                inferred_hidden = int(v.shape[0]); break
            ...
            if inferred_hidden is not None:
                try:
                    ...
                    env.set_inference_policy_state([env.learnable_agent_key], model_state, seq_len=seq_try, hidden_dim=int(inferred_hidden))
                    env.set_model_policy_deterministic(True)
                    env.set_override_learnable_policy(True)
                    print('Loaded seeker checkpoint with inferred hidden_dim:', inferred_hidden, 'seq_len:', seq_try)
    ...
    for i in range(steps):
        ...
        try:
            ...
            for ci in range(int(nc)):
                ...
                if int(c.bi) == sbid or int(c.bj) == sbid:
                    contacts.append({'geom1': int(c.geom1), 'geom2': int(c.geom2), 'frame': [float(c.frame[0]), float(c.frame[1]), float(c.frame[2])]})
        except Exception:
            contacts = []
        ...
        if show_progress and i % 25 == 0:
            print(f"step={i} dist={dist:.3f} speed={speed:.3f} dot={dot:.3f} applied={rec['applied_forward']:.3f} last_ctrl={rec['dbg_last_ctrl_f']:.3f} vx={rec['agent_vx']:.3f} xfrc={rec['xfrc_applied']}")
