...
# 
# 【概要】
# main17_transformer.py (CleanRL) をベースに、ボトルネック分析に基づいた
# 「コードレベルの最適化」を施したバージョンです。
#
# 【主な変更点 (v18.23)】
# 1. バグ修正:
#    - main関数内で UnboundLocalError (envs未定義) が発生していた問題を修正。
#      ObsHistoryの初期化を環境(envs)構築の後に移動。
# 2. ObsHistoryのゼロコピー化 (v18.22維持):
#    - ダブルバッファリングとスライス取得による高速化。
# 3. 実行構成 (維持):
#    - AsyncVectorEnv + スレッド数制限有効化 (Windows/Linux向け)。
#    - 視界判定の正確性重視(距離カットなし)。
#
# 【実行準備】
...
# --- 環境変数設定 ---
# AsyncVectorEnv使用時は、各プロセスのスレッド数を1に制限することで
# CPU競合を防ぎ、トータルのスループットを向上させます。
...
class ObsHistory:
    def __init__(self, num_envs, seq_len, obs_dim, device):
        # バッファ長を2倍確保 (ダブルバッファ)
        self.buffer = torch.zeros((num_envs, seq_len * 2, obs_dim)).to(device)
        self.device = device
        self.ptr = 0
    ...
    def update(self, obs):
        ...
        obs_tensor = torch.from_numpy(obs).to(self.device, dtype=torch.float32, non_blocking=True)
        
        if obs_tensor.ndim == 1:
            obs_tensor = obs_tensor.unsqueeze(0)
            
        # 2箇所に書き込むことで、常に連続したメモリ領域として最新シーケンスを取得可能にする
        # 書き込みコストは2倍になるが、読み出し(get)時のメモリコピーを排除できる
    ...
    def get(self):
        # スライスのみで返す (ゼロコピー)
        # ptr は「次に書き込む場所」=「一番古いデータの場所」を指しているため、
        # ptr から seq_len 分だけ取り出せば、時系列順のデータが得られる
...
class HideAndSeekEnv(gym.Env):
    ...
    # ★修正(v18.15): 遠い順ソート + 遮蔽物キャッシュによる最適化
    def _get_obs(self, agent_id):
        ...
        # --- 最適化: 遠い順に判定して遮蔽物をキャッシュ ---
        self.visible_cache.clear() # インスタンス変数をクリアして再利用 (v18.21)
        ...
        for _, tid in target_dists:
            ...
            is_vis, hit_id = self._is_visible(hp, hra, tp, tid, exclude_body_id=b_id)
            self.visible_cache[tid] = is_vis
            # 見えなくて、かつ何か(hit_id)に遮られた場合、その遮蔽物は「見える」
...
def main():
    ...
    # 観測履歴バッファ
    # ★修正(v18.23): ObsHistoryの初期化を、envs生成後に移動 (UnboundLocalError修正)
    ...
    # ★修正: ここでObsHistoryを初期化 (envsが定義された後)
