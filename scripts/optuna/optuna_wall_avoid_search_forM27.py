...
def objective(trial):
    penalty = trial.suggest_float("rw_wall_avoid_penalty", 0.01, 0.5)
    print(f"[Trial {trial.number}] penalty={penalty:.4f} 開始")
    if not os.path.exists(BACKUP_PATH):
        shutil.copy(TOML_PATH, BACKUP_PATH)
    set_toml_param_in_section("runtime.optuna", "rw_wall_avoid_penalty", penalty)
    try:
...
def main():
    study = optuna.create_study(
        ...
        direction="maximize",
        storage="sqlite:///optuna_wall_avoid.db",
        load_if_exists=True,
    )
    try:
