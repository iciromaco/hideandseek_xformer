...
def run_comprehensive_audit():
    ...
    for scenario in range(2):
        ...
        for i in range(12):
            ...
            print(f" {i:4d}  | {ang:4d}°  | " f"{results[0][i]:10.3f}m | {results[1][i]:10.3f}m | {results[2][i]:10.3f}m | {results[3][i]:10.3f}m | {hit_names[i]}")
        print("-" * len(header))
        ...
        for col_idx, m_idx in enumerate(modes):
            ...
            ax.set_title(
                f"{mode_names[m_idx]} (Scenario {scenario+1})",
                fontsize=14,
                fontweight="bold",
            )
    ...
    for m_idx in modes:
        ...
        if m_idx == 4:
            print(f" {mode_names[m_idx]:15} : {avg_ms:8.4f} ms/call ({sps:10,} SPS) " f"[map_build={mode4_build_sec_reported*1000.0:.2f}ms subtracted]")
        else:
            print(f" {mode_names[m_idx]:15} : {avg_ms:8.4f} ms/call ({sps:10,} SPS)")
    print("-" * 60)

    plt.tight_layout()
    plt.savefig("lidar_visual_audit_v266.png", dpi=150)
...
def run_mode4_cellsize_sweep():
    ...
    header = f"{'cell[m]':>7} | {'M2 ms':>8} | {'M4 ms*':>8} | {'M2 SPS':>8} | {'M4 SPS':>8} | " f"{'M4x':>6} | {'RMSE M4-M1':>11} | {'RMSE M4-M0':>11} | {'build ms':>8}"
    print("-" * len(header))
    print(header)
    print("-" * len(header))
    for r in rows:
    ...
    lines = ["cell_size,m2_ms,m4_ms,m2_sps,m4_sps,speedup_m4_over_m2,rmse_m4_vs_m2,rmse_m4_vs_m0,mode4_build_ms_subtracted"]
    for r in rows:
        lines.append(
