...
from agents.scripted_agents import RuleBasedHider, RuleBasedSeeker, SimpleWanderer
...
def can_see(engine, p1, h1, p2, b_exclude, t_id):
    ...
    if np.dot(v_dir, diff / dist) < math.cos(FOV_HALF_RAD):
        return False
...
def run_match():
    ...
    with mujoco.viewer.launch_passive(m, d) as viewer:
        ...
        while viewer.is_running():
            ...
            s_pos = d.geom_xpos[s_geom][:2].copy()
            s_rot = d.qpos[m.jnt_qposadr[env.qpos_indices["s"]["rot"]]]
            h1_pos = d.geom_xpos[h1_geom][:2].copy()
            h1_rot = d.qpos[m.jnt_qposadr[env.qpos_indices["h1"]["rot"]]]
            h2_pos = d.geom_xpos[h2_geom][:2].copy()

            act_s = s_agent.get_action(
                env._get_obs(0),
                env.vis_engine,
                s_pos,
                s_rot,
            ...
            if hasattr(viewer, "user_scn"):
                ...
                for bid, label, col in [
                    (s_id, f"S:{s_agent.status}", [1, 0, 0, 1]),
                    (h1_id, f"H1:{h1_smart.status}", [0, 1, 0, 1]),
                    (h2_id, "H2:RAND", [0, 0.7, 1, 1]),
                ]:
                    mujoco.mjv_initGeom(
                        ctx.geoms[ctx.ngeom],
                        type=mujoco.mjtGeom.mjGEOM_LABEL,
                        size=[0, 0, 0],
                        pos=d.xpos[bid] + [0, 0, 1.2],
                        mat=np.eye(3).flatten(),
                        rgba=col,
                    )
                    ctx.geoms[ctx.ngeom].label = label
                    ctx.ngeom += 1

                s_c = d.geom_xpos[s_geom].copy()
                s_c[2] = 0.4
                for tid, gid, col in [
                    (h1_id, h1_geom, [1, 0, 0, 1]),
                    (h2_id, h2_geom, [0, 0.7, 1, 1]),
                ]:
                    t_c = d.geom_xpos[gid].copy()
                    t_c[2] = 0.4
