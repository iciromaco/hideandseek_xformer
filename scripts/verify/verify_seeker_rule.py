...
def can_see_target_visual(engine, viewer_pos, viewer_heading, target_pos, body_exclude, target_body_id):
    diff = target_pos - viewer_pos; dist = np.linalg.norm(diff)
...
def run_live_monitor():
    print("="*120)
    print(f"🚀 Seeker Auditor v2.91 (Pure Random Hiders Mode)")
    print("="*120)
    env = TeamCosEnv(); seeker_agent = RuleBasedSeeker()
    hider_agents = [SimpleWanderer(i) for i in range(len(env.hiders))]
    m, d = env.model, env.data; engine = env.vis_engine
    s_key = env.seekers[0]; s_id = env.body_ids[s_key]; s_geom_id = mujoco.mj_name2id(m, mujoco.mjtObj.mjOBJ_GEOM, "seeker_btm")
    h_body_ids = [env.body_ids[hk] for hk in env.hiders]; h_geom_ids = [mujoco.mj_name2id(m, mujoco.mjtObj.mjOBJ_GEOM, f"hider{i+1}_btm") for i in range(len(env.hiders))]

    with mujoco.viewer.launch_passive(m, d) as viewer:
        viewer.cam.distance = 18.0; step_count = 0
        try:
            while viewer.is_running():
                frame_start = time.perf_counter()
                if step_count % 5000 == 0: env.reset()
                
                s_pos_world = d.geom_xpos[s_geom_id][:2].copy(); s_heading = d.qpos[m.jnt_qposadr[env.qpos_indices[s_key]['rot']]]
                h_positions_2d = [d.geom_xpos[gid][:2].copy() for gid in h_geom_ids]
                
                # Seeker: ターゲットの ID リストを渡し、視認判定
                obs_s = env._get_obs(0)
                s_action = seeker_agent.get_action(obs_s, engine, s_pos_world, s_heading, h_positions_2d, s_id, h_body_ids)
                
                # Hiders: シーカーの位置を見せず、自身の座標だけ渡してランダムに動く
                full_action = np.zeros(env.num_agents * 2); full_action[0:2] = s_action
                ...
                if hasattr(viewer, 'user_scn'):
                    ...
                    mujoco.mjv_initGeom(ctx.geoms[0], type=mujoco.mjtGeom.mjGEOM_LABEL, size=np.zeros(3), pos=d.geom_xpos[s_geom_id]+np.array([0,0,1.2]), mat=np.eye(3).flatten(), rgba=[1,1,1,1])
                    ctx.geoms[0].label = f"S:{seeker_agent.status}"; ctx.ngeom = 1
                    s_center = d.geom_xpos[s_geom_id].copy(); s_center[2] = 0.4
                    for side in [-1, 1]:
                        limit_ang = s_heading + (side * FOV_HALF_RAD)
                        end_pt = s_center + np.array([math.cos(limit_ang), math.sin(limit_ang), 0]) * 5.0
                        mujoco.mjv_connector(ctx.geoms[ctx.ngeom], type=mujoco.mjtGeom.mjGEOM_LINE, width=2.0, from_=s_center, to=end_pt)
                        ctx.geoms[ctx.ngeom].rgba = np.array([1, 1, 1, 0.6]); ctx.ngeom += 1
                    for i, h_bid in enumerate(h_body_ids):
                        t_pos = d.geom_xpos[h_geom_ids[i]].copy(); t_pos[2] = 0.4
                        vis, _, _ = can_see_target_visual(engine, s_pos_world, s_heading, t_pos[:2], s_id, h_bid)
                        if vis:
                            mujoco.mjv_connector(ctx.geoms[ctx.ngeom], type=mujoco.mjtGeom.mjGEOM_LINE, width=4.0, from_=s_center, to=t_pos)
                            ctx.geoms[ctx.ngeom].rgba = np.array([1,0,0,1] if i==0 else [0.2, 0.8, 1, 1]); ctx.ngeom += 1
