...
class SimpleWanderer:
    ...
    def get_action(self, obs, current_pos):
        ...
        if self.reflex_timer > 0:
            self.reflex_timer -= 1
            self.status = f"ADJ({self.reflex_timer})"
            if min(lidar[0:3]) > 0.15:
                self.reflex_timer = 0
                return self.current_wander
            act = self.reflex_action.copy()
            if act[0] < 0 and lidar[11] < 0.1:
                act[0] = 0.0
            return act
...
def run_live_monitor():
    print("🚀 verify_seeker_chase v1.67: Stabilized Physical Recovery")
    ...
    with mujoco.viewer.launch_passive(m, d) as viewer:
        ...
        try:
            while viewer.is_running():
                if step_count % EPISODE_LIMIT == 0:
                    env.reset()
                loop_start = time.time()
                obs_s = env._get_obs(0)
                obs_h1 = env._get_obs(1)
                obs_h2 = env._get_obs(2)
                h1_pos, h2_pos, s_pos = (
                    d.xpos[env.body_ids["h1"]][:2].copy(),
                    d.xpos[env.body_ids["h2"]][:2].copy(),
                    d.xpos[env.body_ids["s"]][:2].copy(),
                )
                s_heading = d.qpos[m.jnt_qposadr[env.qpos_indices["s"]["rot"]]]
                s_act = seeker_agent.get_action(
                    obs_s,
                    env.vis_engine,
                    s_pos,
                    s_heading,
                    [h1_pos, h2_pos],
                    body_exclude=env.body_ids["s"],
                )
                h1_act = h1_agent.get_action(obs_h1, h1_pos)
                h2_act = h2_agent.get_action(obs_h2, h2_pos)
                ...
                if hasattr(viewer, "user_scn"):
                    ...
                    for bid, text, color in [
                        (env.body_ids["s"], s_label, [1, 0, 0, 1]),
                        (env.body_ids["h1"], f"H1:{h1_agent.status}", [0, 1, 0, 1]),
                        ...
                        mujoco.mjv_initGeom(
                            ...
                            size=np.zeros(3),
                            pos=d.xpos[bid] + np.array([0, 0, 1.2]),
                            mat=np.eye(3).flatten(),
                            rgba=np.array(color),
                        )
                        viewer.user_scn.geoms[viewer.user_scn.ngeom].label = text
