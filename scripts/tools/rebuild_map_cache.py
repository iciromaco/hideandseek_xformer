...
def generate_main18_cache(output_path="hns_sightmap_v25_cache_Maze.pkl"):
    if XML_CONTENT is None: return False
    ...
    for i in range(model.ngeom):
        ...
        if model.body_jntnum[bid] == 0:
            ...
            if name and "floor" in name: continue
            wall_data.append([data.geom_xpos[i][0], data.geom_xpos[i][1], model.geom_size[i][0], model.geom_size[i][1]])
    ...
    sdf_map = compute_static_sdf_map_jit(grid_coords, np.array(wall_data, dtype=np.float32), np.array(wall_mats, dtype=np.float32)).reshape((n_side, n_side))

    with open(output_path, "wb") as f:
        pickle.dump({"layout_name": "main18", "bounds": bounds, "cell_size": cell_size, "n_side": n_side, "sdf_map": sdf_map}, f)
    print("✅ 完了")
    os.remove(path)
    return True
