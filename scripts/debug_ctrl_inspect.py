...
for val, name in [(1.0, "forward"), (-1.0, "backward")]:
    ...
    try:
        ...
        print("_body_speed_xy error:", e, "vadr=", vadr, "qvel_len=", qlen, "qvel_slice=", qslice)
        vel = float("nan")
    print(f"sent {name} ({val}) -> action_indices_set: {[i for i in range(len(a)) if a[i]!=0]} ctrl[first 12]={ctrl[:12]} vel_xy={vel if not np.isnan(vel) else 'nan'} pos={env.data.xpos[bid].copy()} reward={r}")
