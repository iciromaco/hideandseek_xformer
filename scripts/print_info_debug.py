...
def inspect_target(target):
    ...
    if isinstance(info, dict):
        ...
        for k in keys:
            v = info[k]
            print(f"key={k} type={type(v)} sample=", (v if isinstance(v, (int,float,str)) else (str(type(v)))))
        ...
        for i, item in enumerate(info):
            ...
            for k,v in item.items():
                print(f"  {k}: type={type(v)} val=", (v if isinstance(v, (int,float,str)) else str(type(v))))
    else:
