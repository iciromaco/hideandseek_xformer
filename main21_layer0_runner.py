...
# - 報酬ロジックの刷新:
#   - 距離報酬を廃止し、生存報酬のみに変更。
#   - 生存報酬を 0.05 -> 5.0 (100倍) に増額し、生存の価値を最大化。
# - バグ修正:
#   - stepメソッドでの obs 未定義エラーを修正。
#   - start_time 未定義エラーを修正。
#   - ログ出力に EpLen を追加。
#
# 【実行準備】
...
REWARD_SURVIVAL_SCALE = 1.0        # ★生存報酬を大幅増額 (0.05 -> 5.0)
PENALTY_CAPTURE = -100.0           # 捕獲ペナルティ
PENALTY_STAGNATION_FORCE = -1.1    # 停留ペナルティ (念のため残す)

# 親クラス(main18)の報酬設定を無効化
...
class Layer0RunnerEnv(HideAndSeekEnv):
    ...
    def _seeker_rule_based_policy(self):
        ...
        if self.s0_recovery_mode > 0:
            thrust = -0.2; steering = 1.5; self.s0_recovery_mode -= 1
    ...
    def step(self, action):
        ...
        # 距離報酬は廃止 (REWARD_DISTANCE_DIFF_SCALE = 0.0)
        ...
        for i in range(self.data.ncon):
            ...
            if g1 in self.s0_geoms_set:
                if g2 in self.h1_geoms_set: hider_touch_id = 1
                elif g2 in self.h2_geoms_set: hider_touch_id = 2
            elif g2 in self.s0_geoms_set:
                if g1 in self.h1_geoms_set: hider_touch_id = 1
                elif g1 in self.h2_geoms_set: hider_touch_id = 2
        ...
        if captured_self:
            ...
            # ★修正: 生存報酬を大幅に強化 (生存のみを目的とする)
            reward += REWARD_SURVIVAL_SCALE
            
        # 停留ペナルティ (動かないと減点、生存報酬を目減りさせる)
...
def main():
    ...
    # ★修正: グローバル変数をここで初期化（UnboundLocalError対策）
    ...
    if EXECUTION_MODE == "PLAY":
        ...
        actual_num_envs = 1
        obs_history = ObsHistory(actual_num_envs, base_config.TRANSFORMER_SEQ_LEN, env.observation_space.shape[0], device)
    ...
    if base_config.TRACK_WANDB:
        wandb.init(
            ...
            config={"layer": 0, "mode": "runner_survival_only", "reward": "survival_100x"},
            name=run_name,
            monitor_gym=True,
            save_code=True,
        )
    ...
    obs_history = ObsHistory(actual_num_envs, base_config.TRANSFORMER_SEQ_LEN, envs.single_observation_space.shape[0], device)
    
    obs = torch.zeros((NUM_STEPS, actual_num_envs, base_config.TRANSFORMER_SEQ_LEN, envs.single_observation_space.shape[0]), device=device)
    ...
    if remaining_steps <= 0: remaining_steps = TOTAL_TIMESTEPS
    ...
    try:
        for update in tqdm(range(1, num_updates + 1), desc="Updates"):
            ...
            for step in range(NUM_STEPS):
                ...
                obs_history.update(next_obs)
                if base_config.USE_VIEWER: envs.envs[0].unwrapped.render()
    ...
    envs.close()
    writer.close()
    if base_config.TRACK_WANDB: wandb.finish()
