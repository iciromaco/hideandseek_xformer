...
class Agent(nn.Module):
    ...
    def __init__(self, obs_dim_size_val, action_dim_size_val):
        ...
        # 位置エンコーディング
        self.pos_encoder_param_vec = nn.Parameter(
            torch.zeros(1, TRANSFORMER_SEQ_LEN, HIDDEN_DIM)
        )
        
        # Transformer エンコーダ
        transformer_layer_config_ptr = nn.TransformerEncoderLayer(
            d_model=HIDDEN_DIM, 
            nhead=NUM_HEADS, 
            batch_first=True
        )
        ...
        # 出力層
        self.actor_mean_output_node = layer_init(
            nn.Linear(HIDDEN_DIM, action_dim_size_val), 
            std_val_init_ref=0.01
        )
        self.actor_logstd_params_vec = nn.Parameter(
            torch.zeros(1, action_dim_size_val)
        )
        self.critic_value_output_node = layer_init(
            nn.Linear(HIDDEN_DIM, 1), 
            std_val_init_ref=1.0
        )
    ...
    def get_action_and_value(self, input_seq_tensor_ref, provided_action_tensor_val=None):
        ...
        return provided_action_tensor_val, log_prob_value_res, entropy_value_res, estimated_state_value_res
...
class ObsHistory:
    ...
    def __init__(self, envs_count_val, sequence_len_val, observation_dim_val, device_ptr_obj_ref):
        ...
        self.data_history_buffer_tensor = torch.zeros((envs_count_val, self.total_buffer_length_calculated, observation_dim_val), device=device_ptr_obj_ref)
...
class TeamCosEnv(base_config.HideAndSeekEnv):
    ...
    def _get_cached_ray(self, agent_idx, origin_pt_vec, direction_vec_raw, beam_id_val):
        ...
        mj_ray_actual_computed_dist_res = mujoco.mj_ray(self.model, self.data, ray_from_3d_coordinates_vec, ray_dir_3d_coordinates_vec, None, 1, body_to_exclude_idx, hit_geom_id_out_ptr_buffer)
        self.raycast_cache_storage[cache_key_tuple_identifier] = (origin_pt_vec.copy(), angle_rad_val, mj_ray_actual_computed_dist_res, hit_geom_id_out_ptr_buffer[0])
    ...
    def _is_visible(self, origin_pos, origin_rot, target_pos, target_body_id, agent_idx_ref):
        ...
        res_dist_calc_val, hit_geom_id_calc_val = self._get_cached_ray(agent_idx_ref, origin_pos[:2], direction_2d_unit_vec, target_obj_unique_beam_id)
    ...
    def _get_obs(self, agent_id):
        ...
        rotation_matrix_2d_transform_out = np.array([[cos_yaw_val_final, -sin_yaw_val_final], [sin_yaw_val_final, cos_yaw_val_final]])
        ...
        self_state_vector_packet = np.concatenate([
            ...
            [yaw_angle_rad_val_now_ptr, np.cos(yaw_angle_rad_val_now_ptr), np.sin(yaw_angle_rad_val_now_ptr)]
        ])
        ...
        if lidar_data_vector_final_out is None:
            ...
            for i_idx_val_ptr, offset_rad_val_ptr in enumerate(self.lidar_angles):
                ...
                mj_dist_res_val_ptr, _ = self._get_cached_ray(agent_id, current_xy_coords_ptr_val, beam_dir_vec_obj_ptr_res, i_idx_val_ptr + 1000)
                lidar_data_vector_final_out[i_idx_val_ptr] = min(mj_dist_res_val_ptr, 2.5) / 2.5 if mj_dist_res_val_ptr != -1 else 1.0
            self.lidar_array_cache_storage[agent_id] = (current_xy_coords_ptr_val.copy(), yaw_angle_rad_val_now_ptr, lidar_data_vector_final_out.copy())
        ...
        for tid_ptr_val_ref, name_str_ptr_ref in target_bodies_registry_config_list:
            if tid_ptr_val_ref != target_bid_ptr:
                is_actually_vis_bool_res, _ = self._is_visible(self.data.xpos[target_bid_ptr], yaw_angle_rad_val_now_ptr, self.data.xpos[tid_ptr_val_ref], tid_ptr_val_ref, agent_id)
        ...
        def build_relative_obs_unit_block_ref(target_id_val_ref_ptr, lock_status_bool_flag_ref=None):
            ...
            if is_seen_current_status_flag_res:
                ...
                packet_parts_list_res = [
                    ...
                    [np.cos(yaw_target_absolute_val_ptr_ref - yaw_angle_rad_val_now_ptr), np.sin(yaw_target_absolute_val_ptr_ref - yaw_angle_rad_val_now_ptr)]
                ]
        ...
        if agent_id == 0:
            ...
            final_observation_packet_vector_out = np.concatenate([
                ...
                build_relative_obs_unit_block_ref(self.box2_body, self.locked_boxes[self.box2_body]), 
                build_relative_obs_unit_block_ref(self.ramp_body), 
                rel_enemy_block_info_res, rel_partner_block_info_res, [is_currently_grasping_status_res]
            ])
    ...
    def step(self, action_vector_ptr_input_val):
        ...
        for h_idx_ptr_iter_idx_val_ptr, bid_ptr_val_iter_ptr_ref_ptr in [(1, self.h1_body), (2, self.h2_body)]:
            ...
            if is_seen_v_bool_val_now_ptr:
                ...
                fv_iter_forward_vector_ptr_out = np.array([np.cos(sr_rad_iter_val_now_ptr_out), np.sin(sr_rad_iter_val_now_ptr_out)])
        ...
        return obs_learner_step_final_ptr_val, float(final_reward_step_result), False, truncated_flag_status_ptr_out, info_step_stats_payload_vector_res_final
    ...
    def render(self, stats=None):
        ...
        if self.render_mode == "human":
            ...
            for b_ptr_r_walker_ref_ptr, g_ptr_r_walker_ref_ptr in [(self.box1_body, self.box1_geom_id), (self.box2_body, self.box2_geom_id)]:
                if self.locked_boxes[b_ptr_r_walker_ref_ptr]:
                    self.model.geom_rgba[g_ptr_r_walker_ref_ptr][:] = [0.8, 0.1, 0.1, 1.0]
                elif any(g_ptr_v_ptr_ref_ptr == b_ptr_r_walker_ref_ptr for g_ptr_v_ptr_ref_ptr in self.grasping.values()):
                    self.model.geom_rgba[g_ptr_r_walker_ref_ptr][:] = [0.1, 0.1, 0.9, 1.0]
                else:
            ...
            if self.viewer.user_scn:
                ...
                def add_line_internal_func_final(p1_vec_v_in_ref_ptr, p2_vec_v_in_ref_ptr, rgba_vec_v_in_ref_ptr):
                    if ctx_scn_ptr_walker_ref_ptr.ngeom < ctx_scn_ptr_walker_ref_ptr.maxgeom:
                        mujoco.mjv_initGeom(ctx_scn_ptr_walker_ref_ptr.geoms[ctx_scn_ptr_walker_ref_ptr.ngeom], type=mujoco.mjtGeom.mjGEOM_LINE, size=np.array([0,0,0]), pos=np.array([0,0,0]), mat=np.eye(3).flatten(), rgba=rgba_vec_v_in_ref_ptr)
                        mujoco.mjv_connector(ctx_scn_ptr_walker_ref_ptr.geoms[ctx_scn_ptr_walker_ref_ptr.ngeom], type=mujoco.mjtGeom.mjGEOM_LINE, width=2.0, from_=p1_vec_v_in_ref_ptr, to=p2_vec_v_in_ref_ptr)
                ...
                def add_label_internal_func_final(p_vec_v_in_ref_ptr, txt_str_v_in_ref_ptr, rgba_vec_v_in_ref_ptr):
                    if ctx_scn_ptr_walker_ref_ptr.ngeom < ctx_scn_ptr_walker_ref_ptr.maxgeom:
                        mujoco.mjv_initGeom(ctx_scn_ptr_walker_ref_ptr.geoms[ctx_scn_ptr_walker_ref_ptr.ngeom], type=mujoco.mjtGeom.mjGEOM_LABEL, size=np.array([0,0,0]), pos=p_vec_v_in_ref_ptr, mat=np.eye(3).flatten(), rgba=rgba_vec_v_in_ref_ptr)
                ...
                for tid_r_v_iter_ptr_ref in [self.box1_body, self.box2_body, self.ramp_body, self.s0_body, self.h2_body]:
                    if tid_r_v_iter_ptr_ref == self.h1_body: continue
                    if self.visible_cache_records_map[1].get(tid_r_v_iter_ptr_ref, False):
                        add_line_internal_func_final(p1_c_v_final + [0, 0, 0.5], self.data.xpos[tid_r_v_iter_ptr_ref] + [0, 0, 0.5], [1, 1, 0, 0.4])
                add_label_internal_func_final(self.data.site_xpos[self.id_h1_label], f"H1 Vis:[{','.join(self.visible_obj_names_log_registry[1])}]", [1, 1, 0, 1])

                # H2 (Cyan)
                p2_c_v_final = self.data.xpos[self.h2_body]
                for tid_r_v_iter_ptr_ref in [self.box1_body, self.box2_body, self.ramp_body, self.s0_body, self.h1_body]:
                    if tid_r_v_iter_ptr_ref == self.h2_body: continue
                    if self.visible_cache_records_map[2].get(tid_r_v_iter_ptr_ref, False):
                        add_line_internal_func_final(p2_c_v_final + [0, 0, 0.5], self.data.xpos[tid_r_v_iter_ptr_ref] + [0, 0, 0.5], [0, 1, 1, 0.4])
                add_label_internal_func_final(self.data.site_xpos[self.id_h2_label], f"H2 Vis:[{','.join(self.visible_obj_names_log_registry[2])}]", [0, 1, 1, 1])
                ...
                for tid_r_v_iter_ptr_ref in [self.h1_body, self.h2_body]:
                    if self.visible_cache_records_map[0].get(tid_r_v_iter_ptr_ref, False):
                        add_line_internal_func_final(ps_c_v_final + [0, 0, 0.5], self.data.xpos[tid_r_v_iter_ptr_ref] + [0, 0, 0.5], [1, 0, 0, 0.6])
                add_label_internal_func_final(self.data.site_xpos[self.id_s_label], f"S:{self.seeker_mode}", [1, 0, 0, 1])
...
def load_model_safely(model_obj_ptr_val_ref_ptr, base_name_val_str_ref_ptr, target_type_val_str_ref_ptr):
    """ 指定候補からモデルをロード。 """
    search_paths_array_list_ptr_ref_ptr = [f"{base_name_val_str_ref_ptr}_refinement_{target_type_val_str_ref_ptr}.pt", f"{base_name_val_str_ref_ptr}_initial_{target_type_val_str_ref_ptr}.pt", f"{base_name_val_str_ref_ptr}_{target_type_val_str_ref_ptr}.pt"]
...
def main():
    ...
    try:
        ...
        print(f"--- [Parent] startup failed: {e_parent_startup_ptr_ref_ptr} ---", flush=True)
        sys.exit(1)
    ...
    if EXECUTION_MODE == "PLAY":
        ...
        agent_p_net_obj_res_ref_ptr = Agent(env_play_v_ptr_obj_ref_ptr.observation_space.shape[0], env_play_v_ptr_obj_ref_ptr.action_space.shape[0]).to(device_ptr_final_hardware_ref_final_ptr)
        load_model_safely(agent_p_net_obj_res_ref_ptr, EXPERIMENT_BASE_NAME, TRAIN_TARGET)
        agent_p_net_obj_res_ref_ptr.eval()
        hist_p_manager_ptr_obj_ref_ptr = ObsHistory(1, TRANSFORMER_SEQ_LEN, env_play_v_ptr_obj_ref_ptr.observation_space.shape[0], device_ptr_final_hardware_ref_final_ptr)
        try:
            while True:
                ...
                while not done_play_v_iter_res_ref_ptr:
                    ...
                    o_next_p_step_ptr_res_ref_ptr, r_p_step_ptr_res_ref_ptr, term_p_step_ptr_res_ref_ptr, trunc_p_step_ptr_res_ref_ptr, i_p_step_ptr_res_ref_ptr = env_play_v_ptr_obj_ref_ptr.step(action_play_v_result_ref_ptr.cpu().numpy()[0])
                ...
                print(f"Result -> Return: {ret_play_v_accum_res_ref_ptr:.1f}, Hidden: {i_p_step_ptr_res_ref_ptr['hidden_steps']:.0f}", flush=True)
                sys.stdout.flush()
    ...
    if TRACK_WANDB:
        wandb.init(project=base_config.WANDB_PROJECT_NAME, config={"Target": TRAIN_TARGET, "MODE": MODE, "v": "25.62_StandardizedNames"}, name=unique_exp_run_unique_id_str_final_ptr, sync_tensorboard=False, save_code=True)

    tensorboard_writer_ptr_instance_ref_ptr = SummaryWriter(f"runs/{unique_exp_run_unique_id_str_final_ptr}")
    agent_train_net_ptr_instance_ref_ptr = Agent(vec_envs_ptr_obj_ref_final_ptr.single_observation_space.shape[0], vec_envs_ptr_obj_ref_final_ptr.single_action_space.shape[0]).to(device_ptr_final_hardware_ref_final_ptr)
    ...
    batch_obs_storage_final_tensor_ref_ptr = torch.zeros((S_r_ref_ptr, E_r_ref_ptr, TRANSFORMER_SEQ_LEN, O_r_ref_ptr), device=device_ptr_final_hardware_ref_final_ptr)
    batch_actions_storage_final_tensor_ref_ptr = torch.zeros((S_r_ref_ptr, E_r_ref_ptr, A_r_ref_ptr), device=device_ptr_final_hardware_ref_final_ptr)
    ...
    updates_total_needed_count_res_ref_ptr = int(max(1, (TOTAL_TIMESTEPS - global_s_index_total_val_ref_ptr) // (E_r_ref_ptr * S_r_ref_ptr)))
    ...
    try:
        for u_idx_iter_val_ref_ptr in tqdm(range(1, updates_total_needed_count_res_ref_ptr + 1), desc="Updates"):
            for step_idx_iter_rollout_ref_ptr in range(S_r_ref_ptr):
                ...
                with torch.no_grad():
                    action_out_v_ptr_res_ref_ptr, lp_out_v_ptr_res_ref_ptr, _, value_out_v_ptr_res_ref_ptr = agent_train_net_ptr_instance_ref_ptr.get_action_and_value(rollout_history_buffer_logic_ref_ptr.get())
                ...
                next_obs_v_step_array_res_ref_ptr, reward_v_step_array_res_ref_ptr, term_v_step_array_res_ref_ptr, trunc_v_step_array_res_ref_ptr, info_v_step_dict_res_ref_ptr = vec_envs_ptr_obj_ref_final_ptr.step(action_out_v_ptr_res_ref_ptr.cpu().numpy())
            ...
            with torch.no_grad():
                ...
                for t_idx_ptr_v_rollout_ref_ptr in reversed(range(S_r_ref_ptr)):
                    ...
                    delta_v_calc_res_final_res_ptr = batch_rewards_storage_final_tensor_ref_ptr[t_idx_ptr_v_rollout_ref_ptr] + 0.99 * vp_v_val_ref_final_res_ptr * nt_v_val_ref_final_res_ptr - batch_values_storage_final_tensor_ref_ptr[t_idx_ptr_v_rollout_ref_ptr]
            ...
            for ep_inner_iter_res_ref_ptr in range(UPDATE_EPOCHS):
                ...
                for start_ptr_idx_rollout_ref_ptr in range(0, S_r_ref_ptr * E_r_ref_ptr, MINIBATCH_SIZE):
                    ...
                    _, n_lp_v_unroll_res_ref_ptr, e_v_v_unroll_res_ref_ptr, n_v_val_v_unroll_res_ref_ptr = agent_train_net_ptr_instance_ref_ptr.get_action_and_value(f_obs_unrolled_res_ref_ptr[mb_idx_ptr_unrolled_res_ref_ptr], f_act_unrolled_res_ref_ptr[mb_idx_ptr_unrolled_res_ref_ptr])
            ...
            explained_var_unrolled_final_ptr_ref_ptr = np.nan if var_y_unrolled_final_ref_ptr == 0 else 1 - np.var(y_actual_unrolled_final_ref_ptr - y_pred_unrolled_final_ref_ptr) / var_y_unrolled_final_ref_ptr
            ...
            if history_returns_list_accumulator_ref_ptr:
                ...
                if (TRIAL_MODE) or (u_idx_iter_val_ref_ptr % 10 == 0):
                    ...
                    sps_now_ptr_val_unroll_res_ref_ptr = int((global_s_index_total_val_ref_ptr - start_s_index_val_ptr_res_ref_ptr) / d_wall_clock_ptr_val_final_ref_ptr) if d_wall_clock_ptr_val_final_ref_ptr > 0 else 0
                    
                    print(f"Update {u_idx_iter_val_ref_ptr}, Step {global_s_index_total_val_ref_ptr}, SPS: {sps_now_ptr_val_unroll_res_ref_ptr}, EpRet: {avg_r_s_final_ref_ptr:.1f}, Hidden: {avg_h_s_final_ref_ptr:.1f}, Caught: {avg_c_s_final_ref_ptr:.1f}", flush=True)
                    sys.stdout.flush() 
                    ...
                    tensorboard_writer_ptr_instance_ref_ptr.add_scalar("charts/SPS", sps_now_ptr_val_unroll_res_ref_ptr, global_s_index_total_val_ref_ptr)
    ...
    if SAVE_MODEL :
        ...
        with open(checkpoint_json_ptr_path_final_res_ptr, 'w') as f_out_ptr_final_json_file_ref_ptr:
            json.dump({'global_step': global_s_index_total_val_ref_ptr}, f_out_ptr_final_json_file_ref_ptr)
