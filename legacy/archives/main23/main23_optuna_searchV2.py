...
def save_best_callback(study, trial):
    ...
    try:
        ...
        if study.best_trial.number == trial.number and study.best_trial.value is not None:
            pass
...
def objective(trial, pbar_trials=None):
    ...
    # パラメータ値を表示
    print(
        f"  LR={lr:.1e} ENT={ent:.1e} HB={hidden_bonus:.2f} DS={dist_diff_scale:.2f}",
        flush=True,
    )
    ...
    with open(log_file_path, "w", encoding="utf-8") as log_file:
        ...
        # ★Update進捗を30 Updateごとに表示（コンパクト出力）
        last_shown = 0
        ...
        while process.poll() is None:
            ...
            # 30 Update ごと、または45秒経過で更新
            now = time.time()
            update_delta = current - last_shown
            time_delta = now - last_refresh_time

            if update_delta >= 30 or time_delta >= 45.0:
                # 短縮表示: Update数（総数付き）とHidden値
                print(f"  → U={current}/150 H={hidden:.1f}", flush=True)
                last_shown = current
...
def main():
    ...
    print("Starting Hyperparameter Optimization (v2.8)...")
    print(f"  Mode:           {MODE}")
    ...
    for trial_num in range(N_TRIALS):
        ...
        value = objective(trial, None)  # pbar_trialsをNoneに
