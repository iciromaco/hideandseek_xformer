...
def plot_visibility_with_objects(px, py, data, output_path: Path):
    ...
    im = plt.imshow(vis_row, extent=[-bounds, bounds, -bounds, bounds], 
                    cmap='viridis', origin='upper', alpha=0.6)
    
    # 2. 静的な壁をプロット
    walls = data.get("walls", [])
    for i, wall in enumerate(walls):
        p1, p2 = wall
        plt.plot([p1[0], p2[0]], [p1[1], p2[1]], 'r-', linewidth=3, label="Static Walls" if i == 0 else "")
    ...
    for obj in dyn_objs:
        ...
        size = obj["size"]
        # 矩形として描画
        rect = plt.Rectangle((pos[0]-size[0], pos[1]-size[1]), size[0]*2, size[1]*2, 
                             angle=0, color='orange', alpha=0.8, label=f"Dynamic: {obj['name']}")
        plt.gca().add_patch(rect)
        plt.text(pos[0], pos[1], obj['name'].split('_')[0], color='white', ha='center', va='center', fontsize=8)

    # 4. エージェント（視点）
    plt.plot(px, py, 'ro', markersize=15, markeredgecolor='white', label=f"Agent Viewpoint")
