...
def build_sdf_distance_field(visibility_engine, cell_centers: np.ndarray) -> np.ndarray:
    ...
    print(
        f"[SDF] Building distance field for {num_cells} cells ({grid_size}x{grid_size}) (this may take a few minutes)...",
        flush=True,
    )
    ...
    for i, cell_pos in enumerate(cell_centers):
        ...
        if (i + 1) % max(1, num_cells // 20) == 0:
            ...
            print(
                f"  Progress: {progress*100:.1f}% ({i+1}/{num_cells}), ETA: {eta:.0f}s",
                flush=True,
            )
...
@njit(cache=True, fastmath=True)
def ray_circle_intersection_numba(start_x, start_y, dir_x, dir_y, circle_x, circle_y, radius):
    """
...
def get_2d_geom_segments(model: mujoco.MjModel, data: mujoco.MjData, geom_names: List[str]) -> List[Dict]:
    ...
    for geom_name in geom_names:
        ...
        if "ramp" in geom_name:
            # ランプの上面4頂点のXY投影
            local_top = np.array([[-sx, -sy, sz], [sx, -sy, sz], [sx, sy, sz], [-sx, sy, sz]])
            world_2d = []
            for lc in local_top:
        ...
        # 箱の場合：4つの側面のみ（上下を除く）
            ...
            # 下面の4頂点
            local_bottom = np.array([[-sx, -sy, -sz], [sx, -sy, -sz], [sx, sy, -sz], [-sx, sy, -sz]])
            # 上面の4頂点
            local_top = np.array([[-sx, -sy, sz], [sx, -sy, sz], [sx, sy, sz], [-sx, sy, sz]])
    ...
    def cast_ray(
        ...
        if LIDAR_RAYCAST_MODE == "direct_intersection":
            ...
            dist, hit = cast_ray_direct_numba(
                ...
                self.dynamic_radii,
                self.dynamic_body_ids_array,
                self.num_dynamic_objects,
                self.wall_segments,
                self.num_wall_segments,
                exclude_id1,
                exclude_id2,
                self.max_dist,
            )
...
class Agent(nn.Module):
    ...
    def __init__(self, obs_dim, action_dim):
        ...
        # 位置エンコーディング
        self.pos_enc = nn.Parameter(torch.zeros(1, TRANSFORMER_SEQ_LEN, HIDDEN_DIM))
        ...
        if is_play_mode:
            ...
            if TRAIN_TARGET == "HIDER":
                ...
                if h_path:
                    if should_log:
                        print(
                            f"[Refinement] Loaded Hider model for NPC: {h_path}",
                            flush=True,
                        )
                else:
        ...
        if self.render_mode == "human":
            ...
            if self.viewer.user_scn:
                ...
                add_label(
                    ...
                    f"S:{self.seeker_mode} Vis:[{','.join(self.visible_names[0])}]",
                    [1, 0, 0, 1],
                )
...
def main():
    ...
    def signal_handler(signum, frame):
        ...
        print(
            "\n[SHUTDOWN] Received signal, saving model and closing environments...",
            flush=True,
        )
        ...
        if state_ref["vec_envs"] is not None:
            try:
                ...
                print(
                    "[SHUTDOWN] Environment pipes already broken, forcing exit.",
                    flush=True,
                )
            except Exception as e:
    ...
    if EXECUTION_MODE == "PLAY":
        ...
        if model_path:
            ...
            print("[PLAY] ✗ Main model not found")
        model.eval()
        print("[PLAY] Environment NPC models: (see above for details)")
    ...
    if LOAD_EXISTING_MODELS:
        ...
        print("[Training] Loading agent model...")
        ...
        if model_path:
            ...
            print("[Training] ✗ Agent model not found, training from scratch")
    else:
        print("[Training] LOAD_EXISTING_MODELS=False, training from scratch")
