# main22_reward_optimizationV5.py
...
def create_trial_script(trial, output_dir, mode="initial"):
    ...
    # 1. 生存報酬：最適値 15.4(GBR) ~ 18.4(GPR) をカバーする狭い範囲に絞る
    # Previous: 10.0 ~ 22.0 -> New: 13.0 ~ 20.0
    reward_survival = trial.suggest_float("REWARD_SURVIVAL_SCALE", 1.0, 1.0)

    # 2. 距離報酬：GBR推奨の2.1付近と、他モデル推奨の0付近の両方を検証できる低範囲
    # Previous: 0.5 ~ 10.0 -> New: 0.0 ~ 5.0
    reward_dist_diff = trial.suggest_float("REWARD_DISTANCE_DIFF_SCALE", 0.5, 0.5)

    # 3. 停滞ペナルティ：GBR推奨の -2.6 を中心に高密度探索
    # Previous: -4.0 ~ -1.0 -> New: -3.5 ~ -1.5
    penalty_stagnation = trial.suggest_float("PENALTY_STAGNATION_FORCE", -3.5, -1.5)

    # 4. 学習率：1e-3 (0.001) が最適であることが判明。その周辺をピンポイントで探索
    # logスケールからlinearスケールに近い狭い範囲へ
    # Previous: 5e-4 ~ 5e-3 -> New: 0.0007 ~ 0.002 (7e-4 ~ 2e-3)
    lr = trial.suggest_float("LEARNING_RATE", 7e-4, 2e-3, log=True)

    # 5. エントロピー：1e-5 付近で確定。
    # Previous: 1e-6 ~ 1e-4 -> New: 5e-6 ~ 5e-5 (log=True)
    ent_coef = trial.suggest_float("ENT_COEF", 5e-6, 5e-5, log=True)
