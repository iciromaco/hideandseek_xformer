...
class HideAndSeekEnv(gym.Env):
    ...
    def __init__(
        self,
        train_target=DEFAULT_TRAIN_TARGET,
        render_mode=None,
        save_path_hider=SAVE_PATH_HIDER,
        save_path_seeker=SAVE_PATH_SEEKER,
    ):
    ...
    def _is_safe_pos(self, pos, check_radius, existing_objects=[]):
        ...
        for obj_pos, obj_radius in existing_objects:
            if np.linalg.norm(pos - obj_pos) < (check_radius + obj_radius + 0.2):
                return False
        return True
    ...
    def reset(self, seed=None, options=None):
        ...
        for _ in range(100):
            p = self.np_random.uniform(-4.5, 4.5, size=2)
            if self._is_safe_pos(p, 1.0, [(ramp_pos, 1.5)]):
                box1_pos = p
                break
        ...
        for _ in range(100):
            p = self.np_random.uniform(-4.5, 4.5, size=2)
            if self._is_safe_pos(p, 1.0, [(ramp_pos, 1.5), (box1_pos, 1.0)]):
                box2_pos = p
                break
        ...
        for _ in range(100):
            ...
            if self._is_safe_pos(p, 0.6, existing):
                sx, sy = p[0], p[1]
                break
    ...
    def _apply_action(self, agent_id, action):
        ...
        if action[2] > 0.5:
            if self.grasping[agent_id] is None:
                ...
                target = None
                if d1 < 1.2:
                    target = (self.eq_ids[(agent_id, self.box1_body)], self.box1_body)
                elif d2 < 1.2:
                    target = (self.eq_ids[(agent_id, self.box2_body)], self.box2_body)
        ...
        if self.lock_cooldown[agent_id] <= 0:
            ...
            if target_box:
                ...
                # Unlock
                    ...
                    if box in self.locked_pose:
                        del self.locked_pose[box]
    ...
    def step(self, action):
        ...
        if TRAIN_TARGET == "HIDER":
            ...
            if self.hider_npc_model:
                ...
                act_npc, self.hider_npc_states[partner_id] = self.hider_npc_model.predict(
                    obs_npc,
                    state=self.hider_npc_states[partner_id],
                    episode_start=self.hider_npc_dones[partner_id],
                    deterministic=False,
                )
            ...
            for hid in [1, 2]:
                if self.hider_npc_model:
                    ...
                    act_h, self.hider_npc_states[hid] = self.hider_npc_model.predict(
                        obs_npc,
                        state=self.hider_npc_states[hid],
                        episode_start=self.hider_npc_dones[hid],
                        deterministic=False,
                    )
        ...
        if max(abs(h1p)) > 6.5 or max(abs(h2p)) > 6.5:
            captured = True
    ...
    def _update_seeker_state(self):
        ...
        # CHASING
        if vis1:
            target_pos = self.data.xpos[self.h1_body][:2]
        elif vis2:
            target_pos = self.data.xpos[self.h2_body][:2]
            ...
            if stats and self.viewer.user_scn:
                ...
                for tid, name in targets_common:
                    ...
                    if self._is_visible(
                        ...
                        add_line(
                            h1_pos + np.array([0, 0, 0.5]),
                            self.data.xpos[tid] + np.array([0, 0, 0.5]),
                            np.array([1, 1, 0, 0.6]),
                        )
    ...
    def _on_step(self) -> bool:
        for info in self.locals.get("infos", []):
            if "episode" in info:
                ...
                if len(self.history) % self.plot_every == 0:
                    self._save()
        return True

    def _save(self):
        self.ax.clear()
        self.ax.set_title(f"PPO (LSTM) Progress v15 - Ep: {len(self.history)}")
        y = np.array(self.history)
        self.ax.plot(y, alpha=0.3, color="blue", label="Raw Reward")
        if len(y) >= 10:
