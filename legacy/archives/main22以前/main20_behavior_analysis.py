...
class BehaviorVisualizer:
    ...
    def plot_trajectory_heatmap(self):
        ...
        heatmap, xedges, yedges = np.histogram2d(
            all_positions[:, 0], all_positions[:, 1], bins=60,
            range=[[-ARENA_SIZE/2, ARENA_SIZE/2], [-ARENA_SIZE/2, ARENA_SIZE/2]]
        )
        extent = [xedges[0], xedges[-1], yedges[0], yedges[-1]]
        im = ax.imshow(heatmap.T, extent=extent, origin='lower', cmap='hot', interpolation='bilinear')
    ...
    def plot_distance_evolution(self):
        ...
        ax.axvline(np.mean(avg_dist), color='red', linestyle='--', label=f'Mean: {np.mean(avg_dist):.2f}')
    ...
    def plot_reward_breakdown(self):
        ...
        ax.pie([avg_surv, avg_dist, avg_stag_abs], labels=['Survival', 'Distance', 'Stagnation(abs)'], 
               autopct='%1.1f%%', startangle=90, colors=['lightblue', 'lightgreen', 'lightcoral'])
