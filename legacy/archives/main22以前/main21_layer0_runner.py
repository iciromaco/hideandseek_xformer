...
try:
    ...
    from main18_optimization import HideAndSeekEnv, Agent
except ImportError:
...
class Layer0RunnerEnv(HideAndSeekEnv):
    ...
    def _get_obs(self, agent_id):
        ...
        for _, tid in target_dists:
            if tid in my_vis_cache: continue
            tp_vec = self.data.xpos[tid]; target_pos = tp_vec[:2]
            diff = target_pos - hp; dist = np.linalg.norm(diff)
            if dist < 0.1: is_in_fov = True
            else:
                angle = (np.arctan2(diff[1], diff[0]) - hra + np.pi)%(2*np.pi)-np.pi
                is_in_fov = abs(angle) <= np.deg2rad(FOV_DEG / 2.0)
            if not is_in_fov: my_vis_cache[tid] = False; continue

            cache_key = (agent_id, tid); hit_id = None; should_raycast = True
            if cache_key in self.raycast_cache:
                ...
                if np.linalg.norm(hp - c_hp) < RAYCAST_CACHE_POS_THRESH and np.linalg.norm(target_pos - c_tp) < RAYCAST_CACHE_POS_THRESH:
                    hit_id = c_hid; should_raycast = False; self.raycast_stats["hits"] += 1
            if should_raycast:
                self.raycast_stats["misses"] += 1; dr = np.array([diff[0]/dist, diff[1]/dist, 0.0], dtype=np.float64)
                gid = np.zeros(1, dtype=np.int32); res = mujoco.mj_ray(self.model, self.data, np.array([hp[0], hp[1], 0.5], dtype=np.float64), dr, None, 1, b_id, gid)
                if res != -1:
                    h_body = self.model.geom_bodyid[gid[0]]
                    if h_body == tid: hit_id = tid
                    elif res < dist - 0.4: hit_id = h_body
                    else: hit_id = tid
                self.raycast_cache[cache_key] = (hp.copy(), target_pos.copy(), hit_id)
            
            is_vis = (hit_id == tid)
            my_vis_cache[tid] = is_vis
            if not is_vis and hit_id is not None and hit_id in targets: my_vis_cache[hit_id] = True
        ...
        enemy = get_rel_info(self.s0_body)[:5]; friend = get_rel_info(partner); st = np.array([1.0 if self.grasping[agent_id] else 0.0], dtype=np.float32)
        objs = [get_rel_info(self.box1_body, self.locked_boxes[self.box1_body]), get_rel_info(self.box2_body, self.locked_boxes[self.box2_body]), get_rel_info(self.ramp_body)]
        return np.concatenate([self_s, lidar, *objs, enemy, friend, st]).astype(np.float32)

    def _get_current_dist(self):
        h1p = self.data.xpos[self.h1_body][:2]; h2p = self.data.xpos[self.h2_body][:2]; sp = self.data.xpos[self.s0_body][:2]
    ...
    def _update_seeker_state(self):
        ...
        _ = self._get_obs(0) 
        v1 = self.visible_cache[0].get(self.h1_body, False); v2 = self.visible_cache[0].get(self.h2_body, False)
        if v1 or v2:
            ...
            if np.linalg.norm(sp - self.seeker_last_known_pos) > 0.5:
                self.seeker_target_pos, self.seeker_mode = self.seeker_last_known_pos, "SEARCHING"
    ...
    def _seeker_rule_based_policy(self):
        if self.current_step < base_config.PREP_STEPS: return 0.0, 0.0
        sp, sr = self.data.xpos[self.s0_body][:2], self.data.qpos[self.srot_adr]
        ...
        sf = base_config.SEEKER_RB_THRUST; sr_val = np.clip(ad * 6.0, -3.0, 3.0)
        if abs(ad) > base_config.SEEKER_RB_TURN_THRESH: sf *= 0.3
        if sf > 0.05 and s_vel < 0.05: self.s0_stuck_timer += 5
        else: self.s0_stuck_timer = max(0, self.s0_stuck_timer - 1)
        if self.s0_stuck_timer > 15: self.s0_recovery_mode = 15; self.s0_stuck_timer = 0
        if self.s0_recovery_mode > 0: sf = -0.2; sr_val = 1.5; self.s0_recovery_mode -= 1
        return sf, sr_val
    ...
    def step(self, action):
        ...
        sf0, sr0 = self._seeker_rule_based_policy(); self.data.ctrl[0:2] = [sf0, sr0]
        ...
        for i in range(self.data.ncon):
            c = self.data.contact[i]; n1, n2 = self.model.geom(c.geom1).name, self.model.geom(c.geom2).name
            if ("seeker" in n1 and "hider" in n2) or ("seeker" in n2 and "hider" in n1):
                captured_any = True
                if ("hider1" in n1 or "hider1" in n2) and self.learning_agent_id == 1: captured_self = True
                if ("hider2" in n1 or "hider2" in n2) and self.learning_agent_id == 2: captured_self = True
...
def main():
    ...
    if EXECUTION_MODE == "PLAY":
        ...
        if os.path.exists(model_path):
            agent.load_state_dict(torch.load(model_path, map_location=device)); print(f"Loaded model: {model_path}")
        else: print("Model missing."); return
        agent.eval(); obs_history = ObsHistory(1, base_config.TRANSFORMER_SEQ_LEN, env.observation_space.shape[0], device)
        for ep in range(10):
    ...
    if TRACK_WANDB:
        wandb.init(project=base_config.WANDB_PROJECT_NAME, entity=base_config.WANDB_ENTITY, sync_tensorboard=True, config={"layer": 0, "mode": "runner_v21.56_vis_reward"}, name=run_name, monitor_gym=True, save_code=True)
    writer = SummaryWriter(f"runs/{run_name}")
    def make_env():
        env = Layer0RunnerEnv(); env = gym.wrappers.RecordEpisodeStatistics(env)
        env.get_raycast_stats = env.env.get_raycast_stats; env.reset_raycast_stats = env.env.reset_raycast_stats; return env
    ...
    if LOAD_EXISTING_MODELS and os.path.exists(model_path):
        print(f"★ Loading model: {model_path}"); agent.load_state_dict(torch.load(model_path, map_location=device))
        if os.path.exists(checkpoint_path):
            with open(checkpoint_path, 'r') as f: data = json.load(f); global_step = data.get('global_step', 0); start_global_step = global_step
    ...
    next_done = torch.zeros(NUM_ENVS).to(device); obs_history.reset(); obs_history.update(next_obs)
    ...
    try:
        for update in tqdm(range(1, num_updates + 1), desc="Updates"):
            ...
            for step in range(NUM_STEPS):
                ...
                with torch.no_grad(): action, logprob, _, value = agent.get_action_and_value(obs_history.get()); values[step] = value.flatten()
                actions[step] = action; logprobs[step] = logprob
                next_obs, reward, terminations, truncations, infos = envs.step(action.cpu().numpy())
                next_done = np.logical_or(terminations, truncations); rewards[step] = torch.tensor(reward).to(device).view(-1)
                ...
                if "episode" in infos:
                    ...
                    if isinstance(infos["episode"], dict) and "r" in infos["episode"]:
                        for i, is_done in enumerate(mask):
                            if is_done: episodic_returns.append(infos["episode"]["r"][i]); episodic_lengths.append(infos["episode"]["l"][i])
                obs_history.update(next_obs)
            with torch.no_grad():
                next_v = agent.get_value(obs_history.get()).reshape(1, -1); advantages = torch.zeros_like(rewards).to(device); lastgaelam = 0
            ...
            elapsed = time.time() - start_time; sps = int((global_step - start_global_step) / elapsed) if elapsed > 0 else 0
            writer.add_scalar("charts/SPS", sps, global_step); writer.add_scalar("losses/entropy", entropy.mean().item(), global_step)
            if len(episodic_returns) > 0: writer.add_scalar("charts/mean_episodic_return", np.mean(episodic_returns), global_step); writer.add_scalar("charts/mean_episodic_length", np.mean(episodic_lengths), global_step)
            if update % 10 == 0:
                avg_ret = np.mean(episodic_returns) if episodic_returns else 0.0; avg_len = np.mean(episodic_lengths) if episodic_lengths else 0.0
                print(f"Update {update}, Step {global_step}, SPS: {sps}, Loss: {loss.item():.3f}, EpRet: {avg_ret:.2f}, EpLen: {avg_len:.1f}")
                try:
                    stats_list = envs.call("get_raycast_stats"); total_h = sum(s["hits"] for s in stats_list); total_m = sum(s["misses"] for s in stats_list)
                    if (total_h + total_m) > 0: hr = 100 * total_h / (total_h + total_m); writer.add_scalar("charts/raycast_cache_hit_rate", hr, global_step); print(f"  -> RayCast Cache Hit Rate: {hr:.1f}%")
