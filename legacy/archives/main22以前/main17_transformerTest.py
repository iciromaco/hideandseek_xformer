...
class HideAndSeekEnv(gym.Env):
    def __init__(self, render_mode=None):
        ...
        self.hider_npc_dones = {
            1: np.ones((1,), dtype=bool),
            2: np.ones((1,), dtype=bool),
        }
    ...
    def reset(self, seed=None, options=None):
        ...
        for bid, jid in [
            (self.box1_body, self.box1_joint_id),
            (self.box2_body, self.box2_joint_id),
        ]:
            bp = np.zeros(2)
            for _ in range(100):
                p = self.np_random.uniform(-4.5, 4.5, 2)
    ...
    def _get_obs(self, agent_id):
        ...
        if agent_id == 0:
            h1 = get_rel_info(self.h1_body, "H1")[:5]
            h2 = get_rel_info(self.h2_body, "H2")[:5]
            objs = [
                get_rel_info(self.box1_body, "B1", self.locked_boxes[self.box1_body]),
                get_rel_info(self.box2_body, "B2", self.locked_boxes[self.box2_body]),
                get_rel_info(self.ramp_body, "R"),
            ]
        ...
        enemy = get_rel_info(self.s0_body, "S")[:5]
        friend = get_rel_info(partner, "F")
    ...
    def step(self, action):
        ...
        if TRAIN_TARGET == "HIDER":
            ...
            self.data.ctrl[idx_m : idx_m + 2] = [
                action[0] * HIDER_THRUST_LIMIT,
                action[1],
            ]

            partner = 2 if self.learning_agent_id == 1 else 1
            if self.hider_npc_model:
                act_n, self.hider_npc_states[partner] = self.hider_npc_model.predict(
                    self._get_obs(partner),
                    state=self.hider_npc_states[partner],
                    episode_start=self.hider_npc_dones[partner],
                    deterministic=False,
                )
                self.hider_npc_dones[partner][0] = False
                n_idx = self._apply_action(partner, act_n)
                self.data.ctrl[n_idx : n_idx + 2] = [
                    act_n[0] * HIDER_THRUST_LIMIT,
                    act_n[1],
                ]
            else:
                act_rand = self.action_space.sample() * 0.5
                n_idx = self._apply_action(partner, act_rand)
                self.data.ctrl[n_idx : n_idx + 2] = [
                    act_rand[0] * HIDER_THRUST_LIMIT,
                    act_rand[1],
                ]

            if self.seeker_npc_model:
                act_s, self.seeker_npc_states = self.seeker_npc_model.predict(
                    self._get_obs(0),
                    state=self.seeker_npc_states,
                    episode_start=self.seeker_npc_dones,
                    deterministic=False,
                )
            ...
            for i in [1, 2]:
                if self.hider_npc_model:
                    act_n, self.hider_npc_states[i] = self.hider_npc_model.predict(
                        self._get_obs(i),
                        state=self.hider_npc_states[i],
                        episode_start=self.hider_npc_dones[i],
                        deterministic=False,
                    )
...
def main():
    ...
    # 環境構築
    if TRAIN_MODE:
        envs = gym.vector.SyncVectorEnv([lambda: HideAndSeekEnv(render_mode="human" if i == 0 and USE_VIEWER else None) for i in range(NUM_ENVS)])
    else:
    ...
    # ★修正: ログ初期化
    writer.add_scalar("charts/step", 0, 0)
    writer.flush()
