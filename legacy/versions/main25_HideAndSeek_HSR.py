...
class VisibilityEngine:
    ...
    def get_sdf(self, p):
        ...
        return np.min(np.linalg.norm(np.maximum(d_vec, 0.0), axis=1) + np.minimum(np.max(d_vec, axis=1), 0.0))
...
class TeamCosEnv(gym.Env):
    ...
    def _get_obs(self, agent_id):
        ...
        self._obs_buffer[3:5] = [
            np.cos(self._obs_buffer[2]),
            np.sin(self._obs_buffer[2]),
        ]
