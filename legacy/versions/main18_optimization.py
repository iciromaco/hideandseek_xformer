...
# 演習第18回：パラメータ感度分析に基づく最適化と学習
# 
# 【概要】
# main17_transformer.py (CleanRL) をベースに、ボトルネック分析と
# パラメータ感度分析の結果を反映して最適化したバージョンです。
#
# 【主な実装済み機能】
# 1. 高速化: ObsHistoryゼロコピー, 視界判定キャッシュ, 環境依存スレッド設定
# 2. 安定性: エピソード境界の文脈維持, Transformer警告抑制(enable_nested_tensor=False)
# 3. 利便性: 継続学習(global_step引き継ぎ), Viewerモード対応(actual_num_envs)
# 4. 軽量化: WandBログの間引き(平均値のみ記録)
#
# 【実行準備】
...
# 1. ハイパーパラメータ & 設定
# ==========================================
EXPERIMENT_NAME = "HideAndSeek_Transformer" # 実験名 (モデル保存やWandBに使用)

# ★実験制御スイッチ
LOAD_EXISTING_MODELS = True # True: 既存モデルがあればロード(継続学習), False: 常に新規学習
FIXED_SEED = None               # 整数: シード固定(再現性確保), None: ランダム

if FIXED_SEED is not None:  # シード固定
    SEED = FIXED_SEED       # 整数シード値
else:                  # ランダムシード 
    SEED = int(time.time())
...
TOTAL_TIMESTEPS = 50000000   # 総ステップ数 
LEARNING_RATE = 0.00010748 # 3e-4        # 学習率
...
ENT_COEF = 0.0003     # エントロピー係数           
VF_COEF = 0.5               # 価値関数の損失係数
MAX_GRAD_NORM = 0.5         # 勾配の最大ノルム
TRANSFORMER_SEQ_LEN = 8     # トランスフォーマーのシーケンス長
HIDDEN_DIM = 64             # トランスフォーマー隠れ層次元
NUM_LAYERS = 2              # トランスフォーマーレイヤー数
NUM_HEADS = 2               # トランスフォーマーヘッド数

# 環境設定
TRAIN_TARGET = "HIDER"      # "HIDER" or "SEEKER"  
SAVE_MODEL_PATH = f"{EXPERIMENT_NAME}_{TRAIN_TARGET}.pt"    # モデル保存パス
MODEL_PATH_HIDER = f"{EXPERIMENT_NAME}_HIDER.pt"            # HIDERモデル保存パス
MODEL_PATH_SEEKER = f"{EXPERIMENT_NAME}_SEEKER.pt"          # SEEKERモデル保存パス

# --- 環境定数 ---
ACTION_REPEAT = 16          # アクション繰り返し回数        
PREP_STEPS = 80             # 初期準備ステップ数 
MAX_STEPS = 300             # エピソード最大ステップ数 
FOV_DEG = 135               # 視野角 (度) 
REWARD_SURVIVAL = 0.46989    # 生存報酬 
REWARD_DISTANCE_COEFF = 0.14939 # 距離報酬係数
PENALTY_CAPTURE = -30.0      # 捕獲ペナルティ
PENALTY_STAGNATION = -0.60047   # 停滞ペナルティ
...
SEEKER_RB_THRUST = 0.38      # SEEKERリアクションホイール推力
SEEKER_RB_TURN_THRESH = np.pi/6  # SEEKERリアクションホイール回転閾値

VISIBLE_RADIUS = 15.0         # 視認半径(将来の拡張用)
RAYCAST_CACHE_POS_THRESH = 0.05     # 視界判定キャッシュ位置閾値
# RAYCAST_CACHE_ANGLE_THRESH = np.deg2rad(5.0)  # 視界判定キャッシュ角度閾値`

# ==========================================
# 2. 数学ヘルパー関数
# ==========================================
def quat_inv(q):    # クォータニオンの逆
    return np.array([q[0], -q[1], -q[2], -q[3]])

def quat_mul(q1, q2):   # クォータニオンの積
...
def rotate_vec(q, v):   # ベクトルvをクォータニオンqで回転
    q_vec = np.array([0, v[0], v[1], v[2]])
    q_inv = quat_inv(q)
    return quat_mul(quat_mul(q, q_vec), q_inv)[1:]

def get_body_xy(data, body_id): # ボディのXY位置取得
    return data.xpos[body_id][:2]

def get_body_rot(data, body_id):    # ボディのZ軸回転取得
...
# 3. モデル・バッファ定義
# ==========================================
def layer_init(layer, std=np.sqrt(2), bias_const=0.0):  # レイヤー初期化
    torch.nn.init.orthogonal_(layer.weight, std)
    torch.nn.init.constant_(layer.bias, bias_const)
    return layer

class Agent(nn.Module):  # トランスフォーマーエージェント
    def __init__(self, obs_dim, action_dim):
        super().__init__()
        self.obs_dim = obs_dim  # 観測次元
        self.action_dim = action_dim    # 行動次元
        
        self.embedding = nn.Linear(self.obs_dim, HIDDEN_DIM)    # 埋め込み層
        self.pos_encoder = self._init_positional_encoding()    # 位置エンコーダ
        
        encoder_layer = nn.TransformerEncoderLayer(d_model=HIDDEN_DIM, nhead=NUM_HEADS, batch_first=True)   # トランスフォーマーエンコーダレイヤー
        self.transformer = nn.TransformerEncoder(encoder_layer, num_layers=NUM_LAYERS, enable_nested_tensor=False)   # トランスフォーマーエンコーダ
        
        self.actor_mean = layer_init(nn.Linear(HIDDEN_DIM, self.action_dim), std=0.01)   # アクターメイン出力層
        self.actor_logstd = nn.Parameter(torch.zeros(1, self.action_dim))   # アクター対数標準偏差パラメータ
        self.critic = layer_init(nn.Linear(HIDDEN_DIM, 1), std=1)   # クリティック出力層

    def _init_positional_encoding(self):    # 位置エンコーダ初期化
    ...
    def get_value(self, x):   # 価値関数取得
        x = self.embedding(x) + self.pos_encoder
        x = self.transformer(x)
        return self.critic(x[:, -1, :])

    def get_action_and_value(self, x, action=None): # 行動と価値関数取得
        x = self.embedding(x) + self.pos_encoder
        x = self.transformer(x)
        h = x[:, -1, :] 
...
# ゼロコピーObsHistory  バッファ
class ObsHistory:   # 観測履歴バッファ
    def __init__(self, num_envs, seq_len, obs_dim, device): # 初期化
        self.buffer = torch.zeros((num_envs, seq_len * 2, obs_dim), device=device)
        self.device = device
        self.ptr = 0    # 現在位置ポインタ
        self.seq_len = seq_len  # シーケンス長

    def reset(self):    # バッファリセット
        self.buffer.zero_() # バッファ初期化
        self.ptr = 0    # ポインタリセット

    def update(self, obs):  # 観測追加
        obs_tensor = torch.as_tensor(obs, dtype=torch.float32, device=self.device)  # テンソル変換
        if obs_tensor.ndim == 1:    # バッチ次元追加
            obs_tensor = obs_tensor.unsqueeze(0)    # (1, obs_dim)
            
        self.buffer[:, self.ptr] = obs_tensor   # 現在位置に追加
        self.buffer[:, self.ptr + self.seq_len] = obs_tensor   # シーケンス長分先にも追加
        self.ptr = (self.ptr + 1) % self.seq_len   # ポインタ更新

    def get(self):  # バッファ取得
        return self.buffer[:, self.ptr : self.ptr + self.seq_len]   # 現在位置からシーケンス長分取得
...
<mujoco>
    ...
    <equality>
        <!-- Hider 1 (h1) の把持拘束 -->
        <weld name="eq_grasp_h1_b1" body1="hider1_body" body2="box1_body" active="false" solref="0.08 1" solimp="0.9 0.95 0.001"/>
        <weld name="eq_grasp_h1_b2" body1="hider1_body" body2="box2_body" active="false" solref="0.08 1" solimp="0.9 0.95 0.001"/>
        <weld name="eq_grasp_h1_ramp" body1="hider1_body" body2="ramp_body" active="false" solref="0.08 1" solimp="0.9 0.95 0.001"/>
        
        <!-- Hider 2 (h2) の把持拘束 -->
        <weld name="eq_grasp_h2_b1" body1="hider2_body" body2="box1_body" active="false" solref="0.08 1" solimp="0.9 0.95 0.001"/>
        <weld name="eq_grasp_h2_b2" body1="hider2_body" body2="box2_body" active="false" solref="0.08 1" solimp="0.9 0.95 0.001"/>
        <weld name="eq_grasp_h2_ramp" body1="hider2_body" body2="ramp_body" active="false" solref="0.08 1" solimp="0.9 0.95 0.001"/>
        ...
        <weld name="eq_lock_ramp" body1="world" body2="ramp_body" active="false" solref="0.02 1" solimp="0.95 0.99 0.001"/>
    </equality>
...
class HideAndSeekEnv(gym.Env):
    ...
    def _is_visible(self, origin_pos, origin_rot, target_pos, target_body_id, exclude_body_id):
        ...
        # 距離カットは行わない
        if dist < 0.1: return True, target_body_id 
        
        angle = (np.arctan2(diff[1], diff[0]) - origin_rot + np.pi) % (2*np.pi) - np.pi
        if abs(angle) > np.deg2rad(FOV_DEG / 2.0): return False, None 
    ...
    def _get_obs(self, agent_id):
        ...
        for _, tid in target_dists:
            if tid in self.visible_cache: continue
        ...
        if agent_id == 0:
            h1 = get_rel_info(self.h1_body)[:5]; h2 = get_rel_info(self.h2_body)[:5]
            objs = [get_rel_info(self.box1_body, self.locked_boxes[self.box1_body]), get_rel_info(self.box2_body, self.locked_boxes[self.box2_body]), get_rel_info(self.ramp_body)]
        ...
        enemy = get_rel_info(self.s0_body)[:5]; friend = get_rel_info(partner)
...
def main():
    ...
    if TRAIN_MODE:
        # ★AsyncVectorEnv + スレッド数制限あり (Windows/Linux高速化)
        if not USE_VIEWER:
    ...
    # ★修正: 学習環境数に応じたバッファ初期化
    # USE_VIEWER=True の時は環境数が強制的に1になるため、それに対応
    actual_num_envs = NUM_ENVS if (TRAIN_MODE and not USE_VIEWER) else 1
    
    # ObsHistory初期化 (グローバル定義のクラスを使用)
    obs_history = ObsHistory(actual_num_envs, TRANSFORMER_SEQ_LEN, envs.single_observation_space.shape[0], device)

    agent = Agent(envs.single_observation_space.shape[0], envs.single_action_space.shape[0]).to(device)
    optimizer = optim.Adam(agent.parameters(), lr=LEARNING_RATE, eps=1e-5)
    
    # バッファ類の初期化
    obs = torch.zeros((NUM_STEPS, actual_num_envs, TRANSFORMER_SEQ_LEN, envs.single_observation_space.shape[0]), device=device)
    ...
    global_step = 0
    start_global_step = 0 # 継続学習時の初期ステップ記録用
    ...
    if not TRAIN_MODE:
        ...
        # 1ステップのシミュレーション時間 (秒) = 0.005 * 16 = 0.08s
        ...
        for update in tqdm(range(1, num_updates + 1), desc="Updates"):
            ...
            for epoch in range(UPDATE_EPOCHS):
                ...
                for start in range(0, NUM_STEPS * actual_num_envs, MINIBATCH_SIZE):
                    ...
                    # 損失関数の計算 (エントロピー項は符号に注意: 最小化問題として扱うためマイナス)
            ...
            # ログ出力 (間引きなしの平均値記録 & コンソール出力)
            y_pred, y_true = b_values.cpu().numpy(), b_returns.cpu().numpy()
            var_y = np.var(y_true)
            explained_var = np.nan if var_y == 0 else 1 - np.var(y_true - y_pred) / var_y

            # SPS計算: (現在ステップ - 開始ステップ) / 経過時間
            ...
            # ★修正(v18.60): 以下の詳細ログ出力を削除 (WandB負荷対策)
            ...
            # コンソール出力 (10回に1回でも良いが、tqdmが出ているので毎回でも可。ここでは10回に1回とする)
            if update % 10 == 0:
                ...
                print(f"Update {update}, Step {global_step}, Loss: {loss.item():.3f}, "
                      f"Reward: {rewards.mean().item():.3f}, EpRet: {avg_ep_ret:.2f}, EpLen: {avg_ep_len:.1f}")
                
                # 統計情報の収集 (AsyncVectorEnv時のみ)
                if update % 10 == 0:
                    if not USE_VIEWER: 
                        try:
                            ...
                            if total > 0:
                                ...
                                writer.add_scalar("charts/raycast_cache_hit_rate", hit_rate, global_step)
