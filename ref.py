...
#
# 【主な変更点 (v18.52)】
# 1. ハイパーパラメータの更新 (ユーザー指定設定):
#    - NUM_ENVS=8, NUM_STEPS=128 (大規模並列・データ収集)
#    - HIDDEN_DIM=64, NUM_LAYERS=2 (モデル表現力の向上)
# 2. その他仕様 (維持):
#    - 実験制御スイッチ (LOAD_EXISTING_MODELS, FIXED_SEED)
#    - ObsHistoryのゼロコピー化、視界判定キャッシュ。
#
# 【実行準備】
...
# ★実験制御スイッチ
# 比較実験の公平性を保つため、デフォルトでは既存モデルをロードせず、シードを固定します。
LOAD_EXISTING_MODELS = False # True: 既存モデルがあればロード(継続学習), False: 常に新規学習
FIXED_SEED = 1               # 整数: シード固定(再現性確保), None: ランダム(time.time())
...
# PPO / Transformer (ユーザー指定設定)
TOTAL_TIMESTEPS = 3000000 # 総ステップ数
LEARNING_RATE = 3e-4      # 学習率
NUM_ENVS = 8              # 並列環境数
NUM_STEPS = 128           # 1回の更新で収集するステップ数
MINIBATCH_SIZE = 64       # ミニバッチサイズ (Batch=8*128=1024 なので 64 で16分割)
...
NUM_HEADS = 1             # トランスフォーマーのヘッド数

# 環境設定
TRAIN_TARGET = "HIDER"  # "HIDER" または "SEEKER"
SAVE_MODEL_PATH = f"{EXPERIMENT_NAME}_{TRAIN_TARGET}.pt"   # 学習済みモデル保存パス
MODEL_PATH_HIDER = f"{EXPERIMENT_NAME}_HIDER.pt"           # 学習済みHIDERモデルパス
MODEL_PATH_SEEKER = f"{EXPERIMENT_NAME}_SEEKER.pt"         # 学習済みSEEKERモデルパス
...
FOV_DEG = 135                # 視野角度
REWARD_SURVIVAL = 0.2        # 生存報酬
REWARD_DISTANCE_COEFF = 0.08 # 距離報酬係数
PENALTY_CAPTURE = -30.0      # 捕獲ペナルティ
PENALTY_STAGNATION = -0.5    # 停滞ペナルティ
...
# [最適化] 視認限界距離 (これ以上遠いとRayを飛ばさない)
# 今回は正確性重視のため使用しないが、将来的なチューニング用に定義を残す
VISIBLE_RADIUS = 15.0        # 視認限界距離 (m)

# [最適化] 視界判定キャッシュの閾値 (m)
# この距離以内の移動であれば、前回のRayCast結果を再利用する
...
def main():
    ...
    # ObsHistory初期化
    obs_history = ObsHistory(NUM_ENVS if TRAIN_MODE else 1, TRANSFORMER_SEQ_LEN, envs.single_observation_space.shape[0], device)

    agent = Agent(envs.single_observation_space.shape[0], envs.single_action_space.shape[0]).to(device)
    optimizer = optim.Adam(agent.parameters(), lr=LEARNING_RATE, eps=1e-5)
    
    obs = torch.zeros((NUM_STEPS, NUM_ENVS, TRANSFORMER_SEQ_LEN, envs.single_observation_space.shape[0]), device=device) 
    ...
    # ★修正(v18.53): 外部から置換された変数がfloatになる可能性を考慮し、int()でキャスト
        ...
        for update in tqdm(range(1, num_updates + 1), desc="Updates"):
            ...
            for step in range(NUM_STEPS):
                ...
                if torch.any(next_done):
                    # エピソード終了時: バッファをクリアし、新たな文脈(0埋め)で開始する
                    # これにより前のエピソードの記憶が混入するのを防ぐ
