...
# 【主な変更点 (v18.12)】
# 1. バグ修正:
#    - main関数内で重複定義されていた古いObsHistoryクラスを削除し、TypeErrorを解消。
#      (グローバルスコープの定義が正しく使われるように修正)
# 2. 視界判定の高度な最適化 (v18.11維持):
#    - オブジェクトを「遠い順」に判定し、遮蔽物をキャッシュすることでRayキャストを削減。
# 3. 環境の並列化 (v18.10維持):
#    - AsyncVectorEnv + スレッド数制限有効化。
#
# 【実行準備】
...
class HideAndSeekEnv(gym.Env):
    def __init__(self, render_mode=None):
        ...
        self.npc_obs_history = {
            0: ObsHistory(1, TRANSFORMER_SEQ_LEN, 53), # Seeker
            1: ObsHistory(1, TRANSFORMER_SEQ_LEN, 53), # Hider1
            2: ObsHistory(1, TRANSFORMER_SEQ_LEN, 53), # Hider2
        }

        # ★オーバーヘッド削減：キャッシュ用辞書を環境レベルで保持
        self.visible_cache = {}  # body_id -> bool
    ...
    # ★修正(v18.11): 戻り値を (bool, hit_body_id) に変更
    ...
    def _get_obs(self, agent_id):
        ...
        def get_rel_info(target_id, lock=None):
            # キャッシュチェック
            if target_id in self.visible_cache:
                is_vis = self.visible_cache[target_id]
            else:
                tp = self.data.xpos[target_id]
                # is_visible, hit_body_id を受け取る
                is_vis, hit_id = self._is_visible(hp, hra, tp, target_id, exclude_body_id=b_id)
                self.visible_cache[target_id] = is_vis # ターゲットの結果を保存
