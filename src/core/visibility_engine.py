    ...
    for iy in range(n_y):
        ...
        for ix in range(n_x):
            ...
            if not debug_printed and d_min < -0.2:
                ...
                debug_printed = True
    # compute normals from grid via central differences (numpy gradient)
    # grad_y, grad_x correspond to derivatives along rows (y) and cols (x)
    try:
        grad_y, grad_x = np.gradient(grid.astype(np.float32), cell_size, edge_order=2)
        nx = -grad_x
        ny = -grad_y
        mags = np.sqrt(nx * nx + ny * ny) + 1e-8
        nx = nx / mags
        ny = ny / mags
        normals = np.stack([nx, ny], axis=2).astype(np.float32)
    except Exception:
        normals = np.zeros((n_y, n_x, 2), dtype=np.float32)
...
@njit(cache=True)
def _sample_normal_grid_bilinear(px, py, normal_grid, min_x, min_y, cell_size):
    n_y = normal_grid.shape[0]
    n_x = normal_grid.shape[1]
    gx = (px - min_x) / cell_size
    gy = (py - min_y) / cell_size

    # out-of-bounds: return zero normal
    if gx < 0.0 or gy < 0.0 or gx > (n_x - 1) or gy > (n_y - 1):
        return 0.0, 0.0
    ...
    v00x = float(normal_grid[y0, x0, 0])
    v00y = float(normal_grid[y0, x0, 1])
    v10x = float(normal_grid[y0, x1, 0])
    v10y = float(normal_grid[y0, x1, 1])
    v01x = float(normal_grid[y1, x0, 0])
    v01y = float(normal_grid[y1, x0, 1])
    v11x = float(normal_grid[y1, x1, 0])
    v11y = float(normal_grid[y1, x1, 1])

    vx0 = v00x * (1.0 - tx) + v10x * tx
    vy0 = v00y * (1.0 - tx) + v10y * tx
    vx1 = v01x * (1.0 - tx) + v11x * tx
    vy1 = v01y * (1.0 - tx) + v11y * tx

    rx = vx0 * (1.0 - ty) + vx1 * ty
    ry = vy0 * (1.0 - ty) + vy1 * ty
...
class VisibilityEngine:
    ...
    def sample_sdf_with_normal(self, px, py):
        """Return (distance, normal_x, normal_y) sampled from precomputed SDF and normal grids."""
        self._ensure_mode4_sdf()
        d = _sample_sdf_grid_bilinear(px, py, self._mode4_sdf_grid, self._mode4_min_x, self._mode4_min_y, self._mode4_cell_size, self.max_dist)
        nx, ny = _sample_normal_grid_bilinear(px, py, self._mode4_sdf_normals, self._mode4_min_x, self._mode4_min_y, self._mode4_cell_size)
        return float(d), float(nx), float(ny)
    ...
    def __init__(self, m, d, mode4_sdf_cell_size=SDF_CELL_SIZE):
        ...
        self._mode4_sdf_normals = np.zeros((1, 1, 2), dtype=np.float32)
        ...
        # Precompute and persist static SDF at construction so tools/scripts
        # can rely on an on-disk SDF immediately after env/model creation.
        try:
            self._ensure_mode4_sdf()
        except Exception:
            # If SDF generation fails during init (e.g. partial model not ready),
            # keep lazy generation behavior: leave _mode4_ready=False.
            self._mode4_ready = False
    ...
    def _ensure_mode4_sdf(self):
        ...
        # Ensure MuJoCo has computed world-space geom positions
        try:
        ...
        if os.path.exists(sdf_path):
            ...
            self._mode4_sdf_normals = data.get("normals", np.zeros((self._mode4_sdf_grid.shape[0], self._mode4_sdf_grid.shape[1], 2), dtype=np.float32))
        else:
            self._mode4_sdf_grid, self._mode4_sdf_normals = _compute_wall_sdf_grid_jit(
                float(min_x),
                float(min_y),
                float(cell),
                int(n_x),
                ...
                pickle.dump(
                    {
                        ...
                        "normals": self._mode4_sdf_normals,
