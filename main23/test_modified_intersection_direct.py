...
def ray_segment_intersection_python_debug(start_x, start_y, dir_x, dir_y,
    ...
    if seg_len_sq < 1e-12:
        print(f"  DEBUG: Degenerate segment")
        return 1e6
    ...
    if abs(dir_x) > eps:
        ...
        print(f"  DEBUG: Zero direction vector")
        return 1e6
    
    print(f"  DEBUG: Ray parameter t={t:.4f}")
    
    if t < 1e-6:
        print(f"  DEBUG: Behind ray (t < 1e-6)")
        return 1e6
    ...
    if abs(seg_dx) > eps:
        ...
        print(f"  DEBUG: Degenerate segment (seg_dy, seg_dx both zero)")
        return 1e6
    ...
    if -1e-6 <= u <= 1.0 + 1e-6:
        ...
        print(f"  DEBUG: Out of segment range")
        return 1e6


# Python版の修正版交線法
def ray_segment_intersection_python(start_x, start_y, dir_x, dir_y,
                                     seg_x1, seg_y1, seg_x2, seg_y2):
    """Python版の修正版交線法"""
    seg_dx = seg_x2 - seg_x1
    seg_dy = seg_y2 - seg_y1
...
dist = ray_segment_intersection_python_debug(
    viewpoint[0], viewpoint[1], beam_x, beam_y,
    seg_x1, seg_y1, seg_x2, seg_y2
)
